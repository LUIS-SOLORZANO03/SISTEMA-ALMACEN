...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['agregar_modem'])) {
    ...
    $stmt = $conexion->prepare("INSERT INTO modens_catv (modelo, serial) VALUES (?, ?)");
    if ($stmt) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_id'])) {
    ...
    $stmt = $conexion->prepare("UPDATE modens_catv SET tecnico = ?, fecha = ? WHERE id = ?");
    if($stmt){
...
$result = $conexion->query("SELECT * FROM modens_catv ORDER BY fecha DESC");
?>
...
.table-hover tbody tr:hover { background-color: rgba(0,114,255,0.1);}
...
<h5 class="modal-title">Agregar Modem CA-TV</h5>
